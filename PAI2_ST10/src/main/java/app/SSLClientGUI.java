...
public class SSLClientGUI {
    ...
    private void sendMessage() {
        try {
            ...
            // Enviar acción y mensaje al servidor
            output.println("sendMessage");
            output.println(usernameField.getText().trim()); // Añadimos el usuario que envía el mensaje
            output.println(message);
            output.flush(); // IMPORTANTE: Asegura que los datos se envíen inmediatamente
    
            // Leer respuesta del servidor
            String response = input.readLine();
            if (response != null) {
                responseArea.append("Servidor: " + response + "\n");
            } else {
                responseArea.append("Error: No hay respuesta del servidor.\n");
            }
        } catch (IOException e) {
            responseArea.append("Error al enviar el mensaje.\n");
            e.printStackTrace();
        }
    }
